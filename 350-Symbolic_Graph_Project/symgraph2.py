...
for i in nodes:
    for j in nodes:
        ...
        if condition_1 or condition_2:
            ...
            #TODO: MAPED
            R.add((i,j))
print("\nGraph(Edges):" + str(myBDD_graph))
#print("Graph(setofEdges):" + str(set(myBDD_graph)))
print("R" + str(R) + "\n\n")


#Creates each node on the graph in binary 5 bites per node as per specification
translationGraph = []
myGraph = []
for i in range(0,32):
    tempNum = "{0:b}".format(i)
    if len(tempNum) < 5:
        newNum = ('0' * (5 - len(tempNum))) + tempNum
        translationGraph.append(str(i) + " = " + str(newNum))
        myGraph.append(newNum)
    else:
        translationGraph.append(str(i) + " = " + tempNum)
        myGraph.append(tempNum)
print(translationGraph)
print("\nmyGraph = " + str(myGraph) + "\n")
